      ...
      <el-menu-item v-for="route in topMenus" :key="route.path" :index="route.path">
        <template #title>
          <template v-if="route.icon">
            <el-icon v-if="route.icon.startsWith('el-icon')" class="sub-el-icon">
              <component :is="route.icon.replace('el-icon-', '')" />
            </el-icon>
            <svg-icon v-else :icon-class="route.icon" />
          </template>
...
const activeTopMenuPath =
  route.path.split("/").filter(Boolean).length > 1 ? route.path.match(/^\/[^/]+/)?.[0] || "/" : "/";
 ...
 * @param {string} routePath - 点击的菜单路径
 */
...
onMounted(() => {
  useTimeout(() => {
    console.log(cloneDeep(permissionStore.menuList), "cloneDeep(permissionStore.menuList)");
    topMenus.value = permissionStore.menuList?.length
      ? cloneDeep(permissionStore.menuList)
      : user.getMenu();
  }, 200);
});
</script>
