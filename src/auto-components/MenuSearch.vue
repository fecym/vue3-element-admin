      ...
      <div class="search-result">
        <ul v-if="displayResults.length > 0">
          <li
            ...
            :key="item.path"
            :class="{ active: MenuSearch === activeIndex }"
...
function loadRoutes(routes, parentPath = "") {
  routes.forEach(route => {
    ...
    const routerTitle = route.meta?.title || route.title || route.name;
    if (route.children) {
      loadRoutes(route.children, path);
    } else if (routerTitle) {
      const title = routerTitle === "dashboard" ? "首页" : routerTitle;
...
onMounted(() => {
  ...
  loadRoutes(permissionStore.menuList);
});
</script>
