 ...
 * @property {Object} searchConfig - 搜索配置对象
 */
      ...
      set(value) {
        ...
        inputTagMap[item.prop].data =
          typeof item.attrs?.join === "string"
            ? value.split(item.attrs.join).filter(item => item !== "")
            : value;
      },
    });
  } else {
...
// 关闭标签
function handleCloseTag(prop, tag) {
  inputTagMap[prop].data.splice(inputTagMap[prop].data.indexOf(tag), 1);
}

// 添加标签
function handleInputConfirm(prop) {
