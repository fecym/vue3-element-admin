...
import LayoutSelect from "@/layout/components/Settings/components/LayoutSelect.vue";
import ThemeColorPicker from "@/layout/components/Settings/components/ThemeColorPicker.vue";
...
function changeLayout(layout) {
  ...
  if (layout === LayoutEnum.MIX) {
    ...
    route.path && againActiveTop(route.path);
  }
}
 ...
 */
function againActiveTop(routeName) {
  const parent = findOutermostParent(permissionStore.menuList, routeName);
...
function findOutermostParent(tree, findName) {
  console.log(tree, findName, "???");
  let parentMap = {};

  function buildParentMap(node, parent) {
    // parentMap[node.name] = parent;
    parentMap[node.path] = parent;
  ...
  while (currentNode) {
    ...
    if (!parentMap[currentNode.path]) {
      return currentNode;
    }
    // currentNode = parentMap[currentNode.name];
    currentNode = parentMap[currentNode.path];
  }
